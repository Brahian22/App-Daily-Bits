...
import { urlHtml, urlCss, urlJs, urlFigma, urlUx, urlEstadist } from '../helpers/url'
...
const Questions = () => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      if (category === "html") {
        petiQ = await PeticionData(urlHtml)
        const currentQuizData = petiQ[questionstate.numberQuestion]
        ...
        petiQ = await PeticionData(urlCss)
        const currentQuizData = petiQ[questionstate.numberQuestion]
        setQuestion({
          ...questionstate,
          question: {
        ...
        petiQ = await PeticionData(urlJs)
        const currentQuizData = petiQ[questionstate.numberQuestion]
        setQuestion({
          ...questionstate,
          question: {
        ...
        petiQ = await PeticionData(urlFigma)
        const currentQuizData = petiQ[questionstate.numberQuestion]
        setQuestion({
          ...questionstate,
          question: {
        ...
        petiQ = await PeticionData(urlUx)
        const currentQuizData = petiQ[questionstate.numberQuestion]
        setQuestion({
          ...questionstate,
          question: {
  ...
  const handleSubmit = (e) => {
    ...
    if (questionstate.numberQuestion === petiQ.length) {
      navigate('/estadisticas')
    }

    AgregarEstadisticas(petiQ.length)
  }
  ...
  return (
    <DivForm>
      <DivProgress className="divProgress mb-3" style={{ width: 500, display: "block" }}>
        ...
        <Progress percent={((progress.progressS * 100 / petiQ.length))} size="small"/>
      </DivProgress>
